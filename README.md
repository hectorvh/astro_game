...
Galaxy Drift is a Unity WebGL build under `game-buildV5/`. The Next app embeds it in an iframe at `/game-build/play.html`. Locally, `public/game-build/Build`, `TemplateData`, and `StreamingAssets` are symlinks into `game-buildV5/`. Netlify copies those trees as real files during `pnpm run build:netlify` so the `.wasm` / `.data` are published.
...
| Mini-game | `game-buildV5/` + `public/game-build/play.html` | Unity WebGL (Galaxy Drift / Astro Bunny Runner). |
...
`pnpm run build:netlify` copies `game-buildV5/Build`, `TemplateData`, and `StreamingAssets` into `public/game-build` as real files (the git symlinks would 404 on the CDN), then runs `next build`. The first deploy is large (~95MB Unity `.wasm` + `.data`).
